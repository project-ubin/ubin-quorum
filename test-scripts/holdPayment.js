...
module.exports = (done) => {
  ...
  PaymentAgent.deployed().then((instance) => {
    ...
    return paymentAgent.payments.call(txRef);
      ...
      console.log("holding payment "+txRef+"...");
      return paymentAgent.holdPmt(txRef, {privateFor: keysPrivateFor});    
  }).then((result) => {
    ...
    if (result.logs[0].args.holdStatus) {
      return paymentAgent.holdPmtFromGlobalQueue(result.logs[0].args.txRef,
